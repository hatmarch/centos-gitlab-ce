...
action :create do
  ...
  execute "load sysctl conf #{new_resource.name}" do
    ## Tyrell : Commenting this out, since it breaks the container build.
    ## These kernel optimisations seem to be for PostgreSQL. It is recommended to use
    ## the Openshift dedicated catalog's PostgreSQL running seperatly.
